...
export class IssueWorklogsService {
     ...
     * @throws ApiError
     */
    public getIssueWorklog(
        issueIdOrKey: string,
        startAt?: number,
        maxResults: number = 1048576,
        startedAfter?: number,
     ...
     * @param newEstimate The value to set as the issue's remaining time estimate, as days (\#d), hours (\#h), or minutes (\#m or \#). For example, *2d*. Required when `adjustEstimate` is `new`.
     * @param expand Use [expand](#expansion) to include additional information about worklogs in the response. This parameter accepts `properties`, which returns worklog properties.
     * @param overrideEditableFlag Whether the worklog should be added to the issue even if the issue is not editable. For example, because the issue is closed. Connect app users with admin permission and Forge app users with the `manage:jira-configuration` scope can use this flag.
     * @returns Worklog Returned if the request is successful
     * @throws ApiError
     */
     ...
     * @throws ApiError
     */
    public getIdsOfWorklogsDeletedSince(
        since?: number,
     ...
     * @throws ApiError
     */
    public getIdsOfWorklogsModifiedSince(
        since?: number,
        expand: string = '',
    ): CancelablePromise<ChangedWorklogs> {
        return this.httpRequest.request({
